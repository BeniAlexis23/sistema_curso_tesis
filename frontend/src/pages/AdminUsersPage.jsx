...
const emptyForm = { name: '', lastNames: '', email: '', password: '', roleId: '', isActive: true }
...
export default function AdminUsersPage() {
  ...
  const visible = useMemo(() => users.filter(user =>
    `${user.name} ${user.last_names || ''} ${user.email} ${user.role_name}`.toLowerCase().includes(search.toLowerCase())), [users, search])
  ...
  const openEdit = user => {
    ...
    setForm({ name: user.name, lastNames: user.last_names || '', email: user.email, password: '', roleId: String(user.role_id), isActive: Boolean(user.is_active) })
  }

  const submit = async event => {
    event.preventDefault()
    if (!form.name.trim() || !form.lastNames.trim() || !/^\S+@\S+\.\S+$/.test(form.email) || !form.roleId || (!editingId && form.password.length < 8) || (editingId && form.password && form.password.length < 8)) {
    ...
    setSaving(true)
    try {
      const payload = { ...form, name: form.name.trim(), lastNames: form.lastNames.trim(), email: form.email.trim().toLowerCase(), roleId: Number(form.roleId) }
  ...
  const remove = async user => {
    const result = await Swal.fire({
      ...
      text: `${user.name} ${user.last_names || ''} perderá definitivamente el acceso al panel.`,
      ...
      <form className="review-panel" onSubmit={submit}>
        ...
        <div className="review-body">
          <div className="grid gap-4 sm:grid-cols-2">
            <label><span className="mb-2 block text-sm font-bold text-slate-700">Nombres *</span><input className="min-h-12 w-full rounded-lg border border-slate-300 px-3 outline-none focus:border-undc-cyan" maxLength="120" value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} required /></label>
            <label><span className="mb-2 block text-sm font-bold text-slate-700">Apellidos *</span><input className="min-h-12 w-full rounded-lg border border-slate-300 px-3 outline-none focus:border-undc-cyan" maxLength="120" value={form.lastNames} onChange={event => setForm({ ...form, lastNames: event.target.value })} required /></label>
