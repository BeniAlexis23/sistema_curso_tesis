...
export async function sendApprovalEmail(participant) {
  ...
  await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;color:#17314f;line-height:1.6;max-width:620px"><h2 style="color:#0b4d96">Inscripción confirmada</h2><p>Estimado(a) <strong>${safe.first_names} ${safe.last_names}</strong>:</p><p>Tu inscripción al <strong>Curso Taller de Investigación Aplicada</strong> ha sido aprobada.</p><h3>Datos del participante</h3><ul><li><strong>DNI:</strong> ${safe.dni}</li><li><strong>Correo:</strong> ${safe.email}</li><li><strong>Celular:</strong> ${safe.phone}</li></ul><h3>Información del curso</h3><ul><li><strong>Inicio:</strong> 20 de septiembre de 2026</li><li><strong>Modalidad:</strong> Semipresencial</li><li><strong>Horario:</strong> domingos de 09:00 a 13:30</li></ul><p>Facultad de Ingeniería — Universidad Nacional de Cañete</p></div>`,
  })
  return { sent: true }
}
